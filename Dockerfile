...
RUN wget https://raw.githubusercontent.com/fusionpbx/fusionpbx-install.sh/master/debian/resources/nginx/fusionpbx -O /etc/nginx/sites-available/fusionpbx \ 
	...
	&& find /etc/nginx/sites-available/fusionpbx -type f -exec sed -i 's/nginx.key/pegacorn-communicate-freeswitch.site-a.key/g' {} \; \
	&& find /etc/nginx/sites-available/fusionpbx -type f -exec sed -i 's/nginx.crt/pegacorn-communicate-freeswitch.site-a.crt/g' {} \; \
